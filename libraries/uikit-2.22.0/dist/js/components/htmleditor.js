...
(function(addon) {
    ...
    UI.component('htmleditor', {
        ...
        boot: function() {
            ...
            UI.ready(function(context) {
                ...
                UI.$('textarea[data-i-htmleditor]', context).each(function() {

                    var editor = UI.$(this);

                    if (!editor.data('htmleditor')) {
                        UI.htmleditor(editor, UI.Utils.options(editor.attr('data-i-htmleditor')));
                    }
                });
            });
        },
        ...
        init: function() {
            ...
            this.content    = this.htmleditor.find('.i-htmleditor-content');
            this.toolbar    = this.htmleditor.find('.i-htmleditor-toolbar');
            this.preview    = this.htmleditor.find('.i-htmleditor-preview').children().eq(0);
            this.code       = this.htmleditor.find('.i-htmleditor-code');
            ...
            if (this.options.iframe) {
                ...
                this.iframe = UI.$('<iframe class="i-htmleditor-iframe" frameborder="0" scrolling="auto" height="100" width="100%"></iframe>');
            ...
            this.htmleditor.on('click', '.i-htmleditor-button-code, .i-htmleditor-button-preview', function(e) {

                e.preventDefault();

                if ($this.htmleditor.attr('data-mode') == 'tab') {

                    $this.htmleditor.find('.i-htmleditor-button-code, .i-htmleditor-button-preview').removeClass('i-active').filter(this).addClass('i-active');

                    $this.activetab = UI.$(this).hasClass('i-htmleditor-button-code') ? 'code' : 'preview';
            ...
            this.element.attr('data-i-check-display', 1).on('display.uk.check', function(e) {
        ...
        _buildtoolbar: function() {
            ...
            this.options.toolbar.forEach(function(button) {
                ...
                bar.push('<li><a data-htmleditor-button="'+button+'" title="'+title+'" data-i-tooltip>'+$this.buttons[button].label+'</a></li>');
            });
        ...
        fit: function() {
            ...
            if (mode == 'tab') {
                ...
                this.htmleditor.find('.i-htmleditor-button-code, .i-htmleditor-button-preview').removeClass('i-active')
                    .filter(this.activetab == 'code' ? '.i-htmleditor-button-code' : '.i-htmleditor-button-preview')
                    .addClass('i-active');
            }
    ...
    UI.components.htmleditor.template = [
        '<div class="i-htmleditor i-clearfix" data-mode="split">',
            '<div class="i-htmleditor-navbar">',
                '<ul class="i-htmleditor-navbar-nav i-htmleditor-toolbar"></ul>',
                '<div class="i-htmleditor-navbar-flip">',
                    '<ul class="i-htmleditor-navbar-nav">',
                        '<li class="i-htmleditor-button-code"><a>{:lblCodeview}</a></li>',
                        '<li class="i-htmleditor-button-preview"><a>{:lblPreview}</a></li>',
                        '<li><a data-htmleditor-button="fullscreen"><i class="i-icon-expand"></i></a></li>',
                    '</ul>',
                '</div>',
            '</div>',
            '<div class="i-htmleditor-content">',
                '<div class="i-htmleditor-code"></div>',
                '<div class="i-htmleditor-preview"><div></div></div>',
            '</div>',
        '</div>'
    ].join('');
    ...
    UI.plugin('htmleditor', 'base', {
        ...
        init: function(editor) {
            ...
            editor.addButtons({
                ...
                fullscreen: {
                    title  : 'Fullscreen',
                    label  : '<i class="i-icon-expand"></i>'
                },
                bold : {
                    title  : 'Bold',
                    label  : '<i class="i-icon-bold"></i>'
                },
                italic : {
                    title  : 'Italic',
                    label  : '<i class="i-icon-italic"></i>'
                },
                strike : {
                    title  : 'Strikethrough',
                    label  : '<i class="i-icon-strikethrough"></i>'
                },
                blockquote : {
                    title  : 'Blockquote',
                    label  : '<i class="i-icon-quote-right"></i>'
                },
                link : {
                    title  : 'Link',
                    label  : '<i class="i-icon-link"></i>'
                },
                image : {
                    title  : 'Image',
                    label  : '<i class="i-icon-picture-o"></i>'
                },
                listUl : {
                    title  : 'Unordered List',
                    label  : '<i class="i-icon-list-ul"></i>'
                },
                listOl : {
                    title  : 'Ordered List',
                    label  : '<i class="i-icon-list-ol"></i>'
                }
            ...
            editor.htmleditor.on('click', 'a[data-htmleditor-button="fullscreen"]', function() {
                editor.htmleditor.toggleClass('i-htmleditor-fullscreen');

                var wrap = editor.editor.getWrapperElement();

                if (editor.htmleditor.hasClass('i-htmleditor-fullscreen')) {
        ...
        init: function(editor) {
            ...
            UI.$.extend(editor, {
                ...
                disableMarkdown: function() {
                    ...
                    this.htmleditor.find('.i-htmleditor-button-code a').html(this.options.lblCodeview);
                    this.render();
                }
            ...
            function enableMarkdown() {
                ...
                editor.htmleditor.find('.i-htmleditor-button-code a').html(editor.options.lblMarkedview);
            }
